...
function App() {
  ...
  return (
    ...
    <div className='head'>    <Filter search={search} rating={rating}/>
    <AddMovie addMovie={addMovie}/></div>

    <div> 
         
         <MovieList  movies={movies.filter(el=>el.title.toLowerCase().includes(Keyword.toLowerCase().trim()) && el.rate >= rate )}></MovieList>
    </div>
    </div>
  );
}
